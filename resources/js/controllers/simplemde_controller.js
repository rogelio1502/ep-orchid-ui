...
export default class extends window.Controller {
  static values = { text: String, controls: String }
   ...
   */
  connect () {
    let controls = this.controlsValue.replaceAll('"', '').split('|')
    ...
    controls.forEach(g => {
      g.split(',').forEach(ctrl => {
        expectedToolbar.push(ctrl)
      })
      if (c < controls.length - 1) {
        expectedToolbar.push('|')
      }
      c++
    })
    ...
    let toolbar = []
    expectedToolbar.forEach(e => {
      if (e != '|') {
        toolbar.push(...toolbarOptions.filter(to => to.name == e))
      } else {
        toolbar.push('|')
      }
    })
    ...
    document.onsubmit = e => {
      let _editor = this.editor.value()
      if (!_editor && this.textarea.required) {
        this.editor.element.parentElement.classList.add('border')
        this.editor.element.parentElement.classList.add('border-danger')
        return false
      }
    }
  }
